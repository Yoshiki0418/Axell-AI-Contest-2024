...
class DeepESPCN4x_v2(nn.Module):
    def __init__(self) -> None:
        super().__init__()
        self.scale = 4
        self.conv_1 = nn.Conv2d(in_channels=3, out_channels=32, kernel_size=7, padding=3)
        nn.init.normal_(self.conv_1.weight, mean=0, std=0.001)
        nn.init.zeros_(self.conv_1.bias)
        ...
        self.conv_6 = nn.Conv2d(in_channels=32, out_channels=(4 * self.scale * self.scale), kernel_size=3, padding=1)
        nn.init.normal_(self.conv_6.weight, mean=0, std=0.001)
        nn.init.zeros_(self.conv_6.bias)

        self.pixel_shuffle = nn.PixelShuffle(self.scale)

        self.conv_7 = nn.Conv2d(in_channels=4, out_channels=3, kernel_size=7, padding=3)
